...
foreach ($this->items as $item) {
    $id         = $item['id'];
    $name       = HelperBackend::highlight(isset($this->arrParam['search_value']) ? $this->arrParam['search_value'] : "", $item['username']);
    ...
    $actionBtn  = HelperBackend::showActionButtonsUser($this->arrParam['controller'], $item['id']);
    $slbGroupACP = FormBackend::selectIsNumeric('filter_groupacp', $this->groupName , $this->arrParam['filter_groupacp'] ?? 'default', 'custom-select custom-select-sm mr-1', 'filter_groupacp', 'width: unset');
    $xhtml .= '
    <tr>
        ...
        <td class="text-center position-relative">'.$item['group_name'].'</td>
...
<div class="card card-info card-outline">
    ...
    <?php
        echo $paginationHTM = $this->pagination->showPagination(URL::createLink('admin','user','index'));
    ?>
</div>